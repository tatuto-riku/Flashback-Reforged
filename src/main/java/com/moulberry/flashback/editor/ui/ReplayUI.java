...
public class ReplayUI {
    ...
    public static void init() {
        ...
        Path relativePath = FMLPaths.GAMEDIR.get().relativize(path);
    ...
    public static void initFonts(String languageCode) {
        ...
        byte[] mainFontData = loadFont("inter-medium.ttf");
        if (mainFontData == null) {
            fontConfig.destroy();
            return;
        }
        font = fonts.addFontFromMemoryTTF(mainFontData, size, fontConfig, glyphRanges);

        // Merge in Japanese/Korean/Chinese/etc. characters if needed
        fontConfig.setMergeMode(true);

        fontConfig.setGlyphOffset(0, (int)(5 * getUiScale()));
        byte[] iconFontData = loadFont("materialiconsround-regular.otf");
        if (iconFontData != null) {
            io.getFonts().addFontFromMemoryTTF(iconFontData, (int)(20 * getUiScale()), fontConfig, buildMaterialIconRanges());
        }
        fontConfig.setGlyphOffset(0, 0);

        if (languageCode.startsWith("he")) {
            short[] hebrewRanges = new short[]{(short)'\u0590', (short)'\u05FF', (short)'\uFB1D', (short)'\uFB4F', 0};
            byte[] heeboData = loadFont("heebo-medium.ttf");
            if (heeboData != null) {
                io.getFonts().addFontFromMemoryTTF(heeboData, size, fontConfig, hebrewRanges);
            }
        } else if (languageCode.startsWith("ja")) {
            byte[] jpData = loadFont("notosansjp-medium.ttf");
            if (jpData != null) {
                io.getFonts().addFontFromMemoryTTF(jpData, size*5/4, fontConfig, glyphRanges);
            }
        } else if (languageCode.startsWith("zh")) {
            byte[] tcData = loadFont("notosanstc-medium.ttf");
            if (tcData != null) {
                io.getFonts().addFontFromMemoryTTF(tcData, size*5/4, fontConfig, glyphRanges);
            }
            byte[] scData = loadFont("notosanssc-medium.ttf");
            if (scData != null) {
                io.getFonts().addFontFromMemoryTTF(scData, size*5/4, fontConfig, glyphRanges);
            }
        } else if (languageCode.startsWith("ko")) {
            byte[] krData = loadFont("notosanskr-medium.ttf");
            if (krData != null) {
                io.getFonts().addFontFromMemoryTTF(krData, size*5/4, fontConfig, glyphRanges);
            }
        }
    ...
    private static byte[] loadFont(String name) {
        try {
            ...
            Flashback.LOGGER.warn("Failed to load font: " + name, e);
            return null;
        }
    }
